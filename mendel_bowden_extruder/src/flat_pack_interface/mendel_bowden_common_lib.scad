...
//Bowden cable config
bowden_cable_rad=(6.4 + 0.1) / 2;	// Includes some clearance.
nut_inset_depth = 6.7;	// The depth of the hexoginal hole for the nut to sit in
nut_rad = (11 + 0.2) / 2;			// nut on the bowden cable
...
module box(w,h,d) {
	if (flat_pack == false) scale ([w,h,d]) cube(1, true);
	if (flat_pack == true) scale ([w,h,d]) square(true);
}
...
module m4_hole_vert(l) {
	if (flat_pack == false) cylinder(l,m4_clearance_rad+0.5,m4_clearance_rad+0.7,center=true);
	if (flat_pack == true) circle(m4_clearance_rad+0.5,m4_clearance_rad+0.7,center=true);
}
